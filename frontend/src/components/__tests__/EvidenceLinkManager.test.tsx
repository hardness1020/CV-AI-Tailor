...
describe('EvidenceLinkManager', () => {
  ...
  it('renders evidence links list', () => {
    ...
    expect(screen.getByText('Live Application')).toBeInTheDocument()
  ...
  it('adds evidence link successfully', async () => {
    ...
    // Fill form
    await user.type(screen.getByLabelText(/url/i), 'https://new-link.com')
    await user.selectOptions(screen.getByLabelText(/link type/i), 'website')
  ...
  it('displays correct icons for different link types', () => {
    const linksWithDifferentTypes: EvidenceLink[] = [
      { ...mockEvidenceLinks[0], link_type: 'github' },
      { ...mockEvidenceLinks[1], link_type: 'live_app' },
      { ...mockEvidenceLinks[0], id: 3, link_type: 'document' },
      { ...mockEvidenceLinks[0], id: 4, link_type: 'portfolio' }
    ]
    ...
    // Check that different icons are displayed (emojis in this case)
    expect(screen.getByText('🔗')).toBeInTheDocument() // github
    expect(screen.getByText('🌐')).toBeInTheDocument() // live_app
